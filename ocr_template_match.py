...
for (i, (gX, gY, gW, gH)) in enumerate(locs):  # 遍历每一组大轮廓(包含4个数字)
    ...
    cv_show(group_name, group)
    cv2.imwrite('./credit_card_01/' + group_name + '.jpg', group)
    ...
    for c in digitCnts:  # c表示每个小轮廓的终点坐标
        ...
        for (digit, digitROI) in digits.items():
            # 进行模板匹配, res是结果矩阵
            result = cv2.matchTemplate(roi, digitROI, cv2.TM_CCOEFF)  # 此时roi是5 digitROI是0 依次是1,2.. 匹配10次,看模板最高得分多少
            (_, score, _, _) = cv2.minMaxLoc(result)  # score是最大值
            scores.append(score)  # 10个最大值

        # 得到最合适的数字
        groupOutput.append(str(np.argmax(scores)))  # 取最大值
        z = z + 1
    # 画出来
