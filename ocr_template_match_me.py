...
'''
refCnts = cv2.findContours(ref.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]  # copy是因为以后还要用到ref，用copy以保留
img = cv2.drawContours(img, refCnts, -1, (0, 255, 0), 2)  # 轮廓在二值图上得到, 画是画在原图上（ 参数-1代表画所有的轮廓 ）
